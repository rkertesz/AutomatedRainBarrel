  ...
  if (threeHrVolume < Volume) { // here available volume is greater than expected precip
    ...
    if (useAutoValve) {
      ...
      if (flagOpenValve != 1) {
        ...
        delay(200);
        Serial1.write('O'); //I've just sent a command to open the valve
        // should i write open more than once?
    ...
    while ((Volume < threeHrVolume) && (dist < maxDist)) {
      if (useDrainPump) {
        if (flagOpenValve == 1) {
          if (flagPumpRunning != 1) {
            Serial.println("Run Drainage pump");
            delay(200);
            Serial1.write('P'); //I've just sent a command to run the pump
            flagPumpRunning = 1;
          }
          else {
          ...
          Serial.println("Pump told to run but valve state doens't say open");
        }
      }
      else {
        // no pump, just wait
    ...
    if ((Volume >= threeHrVolume) || (dist >= maxDist)) {
      // we should probably add some hysteresis here to prevent pump or valve cycling
